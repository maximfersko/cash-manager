    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-3 xs:p-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-4 xs:mb-6 sm:mb-8 animate-fade-in">
          <h1 className="text-2xl xs:text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-1 xs:mb-2">{t('register.title')}</h1>
          <p className="text-xs xs:text-sm sm:text-base text-gray-600 dark:text-gray-400">{t('register.subtitle')}</p>
        </div>

        <div className="card p-4 xs:p-5 sm:p-6 animate-slide-up">
          <h2 className="text-xl xs:text-2xl font-semibold text-gray-900 dark:text-white mb-4 xs:mb-5 sm:mb-6">{t('register.signUp')}</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 xs:space-y-4 sm:space-y-5">
            <div>
              ...
              <div className="relative">
                <User className="absolute left-2.5 xs:left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 xs:w-5 xs:h-5 text-gray-400" />
                <input
                  type="text"
                  value={name}
            ...
            <div>
              ...
              <div className="relative">
                <Mail className="absolute left-2.5 xs:left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 xs:w-5 xs:h-5 text-gray-400 dark:text-gray-500" />
                <input
                  type="email"
                  value={email}
            ...
            <div>
              ...
              <p className="mt-1 text-[10px] xs:text-xs text-gray-500 dark:text-gray-400">{t('register.passwordHint')}</p>
            </div>
            ...
            <label className="flex items-start">
              <input type="checkbox" className="mt-0.5 xs:mt-1 rounded border-gray-300 dark:border-gray-600 text-primary-500 focus:ring-primary-500 dark:bg-gray-700 w-3.5 h-3.5 xs:w-4 xs:h-4 flex-shrink-0" />
              <span className="ml-1.5 xs:ml-2 text-[10px] xs:text-xs sm:text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                {t('register.agreeTerms')} <a href="#" className="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-500">{t('register.termsOfService')}</a> {t('register.and')} <a href="#" className="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-500">{t('register.privacyPolicy')}</a>
              </span>
            </label>
          ...
          <div className="mt-4 xs:mt-5 sm:mt-6">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs xs:text-sm">
                <span className="px-2 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400">{t('register.continueWith')}</span>
              </div>
            </div>
